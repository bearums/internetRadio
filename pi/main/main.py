...
#first connect speaker
bluetooth_speaker = False
def connect_bt_speaker():
	status = subprocess.call('ls /dev/input/event0 2>/dev/null', shell=True)

	if status !=0: # 0 means connected  
		while status == 2:
        		print("Bluetooth DOWN")
        		print(status)
        		subprocess.call('/home/pi/radio/playground/bt_connect.sh', shell=True)
        		time.sleep(2)
        		status = subprocess.call('ls /dev/input/event0 2>/dev/null', shell=True)
			
	#change volume to max
	#Popen(['amixer','-D','bluealsa', 'sset', '"UE BOOM 2 - A2DP"', '{}%'.format(100)],stdout=open(os.devnull, 'wb'))
			
	audio_device= 'bluealsa'
	Player=vlc.Instance('--aout=alsa --alsa-audio-device='+ audio_device)
...
for i in range(0, len(name_list)):
	...
	#subprocess.call(['ffmpeg','-nostats','-loglevel','0', '-y', '-i', 'station_name_voices/{}.wav'.format(i), 'station_name_voices/{}.mp3'.format(i)],stdout=open(os.devnull, 'wb'))
	#ml = Player.media_list_new(['station_name_voices/1s_silence.mp3','station_name_voices/{}.mp3'.format(i), station_list[i]])
...
curr_station = 0
global a
a=play_station(curr_station, a_old=None)
while True:
	...
	"""if a5 != btn_1:
		if a5 ==1:
			...
			a.stop()
			time.sleep(1)
			Player = connect_bt_speaker()
			time.sleep(1)
			a=play_station(curr_station, a_old=None, p=Player)
		if a5 ==0:
			print 'btn1 released!'
		btn_1= a5

	#check for change of vol pot
	if a2 != pot_vol:
		pot_vol = a2
		print 'changing volume ', pot_vol
		change_volume(pot_vol, a)
	
	
	#check for change of station pot
        if a1 != pot_station:
                pot_station = a1
                new_station = potval_to_station(pot_station)
                if new_station != curr_station:
					print pot_station ,' changing station to ', new_station
					#a.stop()
					a=play_station(new_station, a )
					curr_station = new_station
	"""
